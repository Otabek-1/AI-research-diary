...
export function readingTime(document: Document) { const words = document.content.map((block) => block.text).join(" ").split(/\s+/).filter(Boolean).length; return Math.max(1, Math.ceil(words / 190)); }
export function sectionLabel(section: string) { return section.split("/").map((part) => part.replace(/-/g, " ").replace(/\b\w/g, (letter) => letter.toUpperCase())).join(" / "); }
